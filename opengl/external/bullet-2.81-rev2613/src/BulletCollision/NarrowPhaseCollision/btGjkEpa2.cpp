	...
	struct	EPA
	{
		...
		struct	eStatus { enum _ {
			...
			bool getedgedist(sFace* face, sSV* a, sSV* b, btScalar& dist)
			{
				...
				const btVector3 n_ab = btCross(ba, face->n); // Outward facing edge normal direction, on task_1_triangle plane
			...
			sFace*				newface(sSV* a,sSV* b,sSV* c,bool forced)
			{
				if(m_stock.root)
				{
					...
					if(v)
					{
						...
						{
							// Origin projects to the interior of the task_1_triangle
							// Use distance to task_1_triangle plane
