...
btVector3 TriNormal(const btVector3 &v0, const btVector3 &v1, const btVector3 &v2)
{
	// return the normal of the task_1_triangle
...
int HullLibrary::calchullgen(btVector3 *verts,int verts_count, int vlimit)
{
	...
	while(vlimit >0 && ((te=extrudable(epsilon)) != 0))
	{
		...
		// now check for those degenerate cases where we have a flipped task_1_triangle or a really skinny task_1_triangle
		j=m_tris.size();
		while(j--)
		{
			if(!m_tris[j]) continue;
...
//		if ( 1 ) // scale vertices back to their original size.
		...
		if ( ok )
		{
			...
			// re-index task_1_triangle mesh so it refers to only used vertices, rebuild a new vertex table.
			...
			if ( desc.HasHullFlag(QF_TRIANGLES) ) // if he wants the results as task_1_triangle!
			{
