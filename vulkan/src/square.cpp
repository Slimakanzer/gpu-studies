...
#ifdef BRICK
#define TEXTURE_PATH  "textures/brick_texture.jpg"
#define NORMAL_MAP_PATH "textures/brick_normal_map.jpg"
#else
#define TEXTURE_PATH "textures/rock_texture.jpg"
#define NORMAL_MAP_PATH "textures/rock_normal_map.jpg"
#endif
...
private:
    ...
    void initVulkan() {
        ...
        createTextureImage(NORMAL_MAP_PATH, textureNormalImage, textureNormalImageMemory);
        createTextureImageView(textureNormalImageView, textureNormalImage);
        createTextureSampler(textureNormalSampler);
        createVertexBuffer(sizeof(vertices[0]) * vertices.size(), vertexBuffer, vertexBufferMemory);
        createIndexBuffer(sizeof(indices[0]) * indices.size(), indexBuffer, indexBufferMemory);
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("shaders/bin/square/vert.spv");
        auto fragShaderCode = readFile("shaders/bin/square/frag.spv");
/*        auto tescShaderCode = readFile("shaders/bin/square/tesc.spv");
        auto teseShaderCode = readFile("shaders/bin/square/tese.spv");*/
...
/*        VkPipelineShaderStageCreateInfo tessControlShaderStageInfo = {};
        ...
        tessControlShaderStageInfo.module = tescShaderModule;
        ...
        tessEvaluationShaderStageInfo.module = teseShaderModule;
...
/*        vkDestroyShaderModule(device, tescShaderModule, nullptr);
    ...
    void createVertexBuffer(VkDeviceSize bufferSize, VkBuffer &vertexBuffer, VkDeviceMemory &vertexBufferMemory) {
    ...
    void createIndexBuffer(VkDeviceSize bufferSize, VkBuffer &indexBuffer, VkDeviceMemory &indexBufferMemory) {
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            if (textureNormalImageView == VK_NULL_HANDLE) {
                throw new std::runtime_error("WTF MAZAFAKA");
            }

            std::vector<VkDescriptorImageInfo> descriptorImageInfos = {imageInfo, imageNormalInfo};
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        static float speedRange = 0.002f;
        static float speedRotation = 5.0f;
        ...
        ubo.model = glm::rotate(glm::mat4(1.0f), angle * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        ubo.view = glm::lookAt(glm::vec3(range, range, range), glm::vec3(0.0f, 0.0f, 0.0f),
                               glm::vec3(0.0f, 0.0f, 1.0f));
        ubo.proj = glm::perspective(glm::radians(45.0f), swapChainExtent.width / (float) swapChainExtent.height, 0.1f,
                                    10.0f);
        ubo.proj[1][1] *= -1;

        ubo.lightPosition = glm::vec3(0, 1, 0);
